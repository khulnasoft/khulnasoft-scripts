	...
	cat <<-EOUSAGE
		...
		usage: $self [options] repo [repo ...]
		   ...
		   ie: $self library/docker
		       ...
		       $self library/docker khulnasoft/docker-master | sort -uV

		       $self --jq '.results[]' khulnasoft/syncthing

		       $self --jq '.results[] | [ .name, (.full_size | tostring), .last_updated ] | join("\\t")' khulnasoft/syncthing | sort -V | column -ts \$'\\t'
